...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('extract_horses.log', encoding='utf-8'),
...
def extract_horse_info(html_file):
    """キャッシュから馬情報を抽出する"""
    logging.info(f"Extracting from {html_file}")
    ...
    for section in horse_sections:
        try:
            ...
            if detail_link:
                try:
                    # 詳細ページのキャッシュを確認
                    cache_dir = os.path.dirname(os.path.dirname(html_file))
...
def main():
    ...
    # 結果を保存
    output_file = os.path.join(latest_cache, 'extracted_horses.json')
