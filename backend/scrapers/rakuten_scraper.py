...
class RakutenAuctionScraper:
    ...
    def scrape_horse_detail(self, detail_url: str, auction_date: Optional[str] = None):
        ...
        try:
            ...
            # オークション詳細ページから総賞金を取得
            prize_start, prize_latest = self._extract_prize_money_from_auction_detail(soup)
            detail_data['total_prize_start'] = prize_start
            detail_data['total_prize_latest'] = prize_latest
            print(f"  オークション詳細から賞金を取得: start={prize_start}万円, latest={prize_latest}万円")
    ...
    def _extract_prize_money_from_auction_detail(self, soup) -> Tuple[float, float]:
        """
        オークション詳細ページから総賞金を抽出する
        
        Args:
            soup: BeautifulSoupオブジェクト
            
        Returns:
            Tuple[float, float]: (total_prize_start, total_prize_latest) のタプル
        """
        try:
            # 総賞金を含む行を探す
            prize_row = None
                ...
                if value_elem:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            return (prize, prize)  # startとlatestは同じ値
                        except ValueError:
                            pass
            
            # 別のパターン: テーブル内の「総賞金」行を探す
            for row in soup.find_all('tr'):
                th = row.find('th')
                td = row.find('td')
                if th and '総賞金' in th.get_text() and td:
            ...
            # 見つからなかった場合はデフォルト値(0.0, 0.0)を返す
            return (0.0, 0.0)
            
        except Exception as e:
            print(f"賞金情報の抽出中にエラーが発生: {e}")
            return (0.0, 0.0)
        ...
        for idx, element in enumerate(horse_elements, 1):
            try:
                ...
                try:
                    ...
                    if price_elem:
                        ...
                        if match:
                            try:
                                ...
                                horse_data['total_prize_latest'] = total_prize
                    ...
                    # JBISから現在の総賞金を取得してtotal_prize_latestに設定
                    if 'jbis_url' in horse_data and horse_data['jbis_url']:
                        try:
                            print(f"JBISから現在の総賞金を取得中: {horse_data['name']}")
                            normalized_jbis_url = self._normalize_jbis_url(horse_data['jbis_url'])
                            if normalized_jbis_url != horse_data['jbis_url']:
                                print(f"    JBIS URL正規化: {horse_data['jbis_url']} -> {normalized_jbis_url}")
                            ...
                            if total_prize_dt and total_prize_dt.find_next_sibling('dd'):
                                ...
                                if match:
                                    ...
                                    print(f"    dtタグから総賞金を取得: {total_prize_latest}万円")
                        ...
                        print("    JBIS URLが存在しないため、賞金の取得をスキップ")
