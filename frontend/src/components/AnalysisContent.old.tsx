...
import { getDisplayPrice, formatPrice } from '@/utils/price';
...
type SortKey = 'name' | 'age' | 'price' | 'roi' | 'weight' | 'prize';
...
export default function AnalysisContent({ data }: AnalysisContentProps) {
  const [tableHorses, setTableHorses] = useState<HorseWithCalculations[]>([]);
  ...
  const [filterBySire, setFilterBySire] = useState<string>('');
  const [filterByDam, setFilterByDam] = useState<string>('');
  const [avgROI, setAvgROI] = useState<number>(0);

  // 馬データを変換する関数
  const transformHorseData = useCallback((horses: any[]): HorseWithCalculations[] => {
  // 明示的に型を指定した空配列を定義
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-4 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700">
          <p>デバッグ情報: 表示中の馬の数: {sortedHorses.length}</p>
          <p>表示タイプ: {showType}</p>
        ...
        <div className="mb-6 text-lg font-semibold text-gray-700 flex flex-wrap gap-8">
          <span>総馬数: {sortedHorses.length}</span>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <div>
            ...
            <div className="flex gap-2">
              ...
              <Button 
                onClick={() => setShowType('sold')} 
                variant={showType === 'sold' ? 'default' : 'outline'}
                size="sm"
              >
                落札済み
              </Button>
              <Button 
                onClick={() => setShowType('unsold')} 
                variant={showType === 'unsold' ? 'default' : 'outline'}
                size="sm"
              >
                未落札
              </Button>
              <Button 
                ...
                variant={showType === 'roi' ? 'default' : 'outline'}
                size="sm"
              >
                ROI順
              </Button>
          ...
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-1">父馬で検索</label>
            <input
              type="text"
              className="border rounded p-2 w-full text-sm"
              placeholder="父馬名を入力"
              value={filterBySire}
              onChange={(e) => setFilterBySire(e.target.value)}
            />
          </div>
          
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-1">母馬で検索</label>
            <input
              type="text"
              className="border rounded p-2 w-full text-sm"
              placeholder="母馬名を入力"
              value={filterByDam}
              onChange={(e) => setFilterByDam(e.target.value)}
            />
          </div>
        </div>
