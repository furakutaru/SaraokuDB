...
const transformHorseData = (data: any): AnalysisHorse[] => {
  ...
  console.log('Input data structure:', data ? 'Valid' : 'Invalid');
  ...
  try {
    ...
    // 馬データの配列を取得（配列そのものか、horsesプロパティの配列か）
    let horsesArray: any[] = [];
    
    if (Array.isArray(data)) {
      // 配列が直接渡された場合
      horsesArray = data;
    } else if (data.horses && Array.isArray(data.horses)) {
      // horsesプロパティに配列がある場合
      horsesArray = data.horses;
    } else if (typeof data === 'object') {
      // 単一の馬オブジェクトが渡された場合
      horsesArray = [data];
    }
    
    console.log(`Found ${horsesArray.length} horses to process`);
  
  // メタデータを保持（存在しない場合は空のオブジェクト）
  ...
  try {
    ...
    const result = filteredHorses.map((horse: any, index: number) => {
      console.log(`[${index}] Mapping horse:`, horse.name || 'No name');
      
      // IDを明示的に文字列に変換（detail_urlからIDを抽出するか、なければインデックスを使用）
      const horseId = horse.detail_url 
        ? horse.detail_url.split('/').pop() 
        : `horse-${Date.now()}-${index}`;
      
      // オークション履歴を取得（horses.jsonには直接のオークション履歴がないため空配列）
      ...
      const getEffectiveWeight = (): number | null => {
        ...
        // 2. オークション履歴から最新の体重を確認（horses.jsonにはオークション履歴がないためスキップ）
    ...
    // 賞金情報を取得
    const totalPrize = horse.prize_money?.total_prize || '0';
    const totalPrizeNumeric = parseInt(totalPrize.replace(/[^0-9]/g, '')) || 0;
    ...
    const defaultAuction: AuctionHistory = {
      ...
      total_prize_start: 0, // 初期賞金は0と仮定
      total_prize_latest: totalPrizeNumeric, // 現在の賞金
      weight: horseWeight,
      seller: horse.seller || '',
      ...
      auction_url: horse.detail_url || '' // オークションURLはdetail_urlを流用
    };
    
    // horses.jsonにはオークション履歴がないため、デフォルトのオークション情報のみを使用
    const latestAuction = defaultAuction;
    ...
    // 2. オークション履歴のsold_priceを確認（horses.jsonにはオークション履歴はないが、念のため）
    if ((soldPrice === null || soldPrice === 0) && latestAuction.sold_price) {
    ...
    // 2. sold_priceがnullまたは0の場合
    const auctionEndDate = horse.auction_date ? new Date(horse.auction_date) : null;
    const isAuctionEnded = auctionEndDate ? auctionEndDate < new Date() : false;
    
    // 主取りフラグの判定
    const hasUnsoldFlag = 
      (horse as any).is_unsold === true || 
      (horse as any).unsold === true ||
      (latestAuction && ((latestAuction as any).is_unsold === true || (latestAuction as any).unsold === true));
    
    // 主取りの最終判定
    const isHorseUnsold = hasUnsoldFlag || 
      (soldPrice === null || soldPrice === 0 || soldPrice === '0' || soldPrice === '');
      
    // デバッグ用ログ
    if (index < 3) {
      console.log(`[${index}] ${horse.name} - Unsold Check:`, {
        hasUnsoldFlag,
        soldPrice,
        isAuctionEnded,
        isHorseUnsold,
    ...
    // デフォルト値で初期化
    const defaultAuctionData: AuctionHistory = {
      id: horseId,
      horse_id: horseId,
    ...
    // オークション履歴が存在する場合はマージ
    const effectiveAuction: AuctionHistory = latestAuction 
      ? {
          ...defaultAuctionData,
          ...latestAuction,
          // 馬オブジェクト直下の情報で上書き
          ...
          detail_url: latestAuction.detail_url || horse.detail_url || horse.auction_url || `#/horse/${horseId}`,
    ...
    // デバッグ用ログ
    if (index < 3) {
      console.log(`[${index}] ${horse.name} - Effective Auction:`, {
        id: effectiveAuction.id,
        auction_date: effectiveAuction.auction_date,
        sold_price: effectiveAuction.sold_price,
        is_unsold: effectiveAuction.is_unsold,
        weight: effectiveAuction.weight
      });
    }
    ...
    const numericSoldPrice = toNumeric(soldPrice);
    const numericPrizeMoney = toNumeric(prizeMoney);
    
    // ROI計算（落札価格が0の場合は0）
    const roi = numericSoldPrice > 0 ? ((numericPrizeMoney - numericSoldPrice) / numericSoldPrice) * 100 : 0;
    
    // デバッグ用ログ
    if (index < 3) {
      console.log(`[${index}] ${horse.name} - Price Calculations:`, {
        soldPrice,
        numericSoldPrice,
        prizeMoney,
        numericPrizeMoney,
    ...
    // 画像URLの正規化
    const normalizeImageUrlSafe = (baseUrl: string, url: string | { image_url?: string } | null | undefined): string => {
      try {
        if (!url) return '';
        
        let imageUrl = '';
        if (typeof url === 'object' && url !== null) {
          imageUrl = (url as any).image_url || '';
    ...
    const imageUrlNormalized = normalizeImageUrlSafe(
    ...
    // デバッグ用ログ
    if (index < 3) {
      console.log(`[${index}] ${horse.name} - Image URL:`, {
        original: horse.image_url,
        normalized: imageUrlNormalized
      });
    }
    ...
    const baseHorse: any = {
      ...
      id: horseId,
      name: horse.name || '不明',
      age: typeof horse.age === 'number' ? horse.age : (parseInt(horse.age as string, 10) || 0),
      sex: horse.sex || '不明',
      sire: horse.sire || '不明',
      dam: horse.dam || '不明',
      ...
      sold_price: soldPrice,
      seller: horse.seller || '',
      weight: effectiveWeight || 0,
      total_prize_start: 0, // 初期値は0に設定
      total_prize_latest: prizeMoney, // 取得した賞金を設定
      
      // 主取り（unsold）の判定
      unsold: isHorseUnsold,
      unsold_count: isHorseUnsold ? 1 : 0, // 簡易的なカウント
      comment: horse.comment || '',
      ...
      is_updated: false,
      is_featured: false,
      is_sold: !isHorseUnsold, // 主取りでない場合は落札済み
      is_unsold: isHorseUnsold,
      is_withdrawn: false,
      is_reserved: false,
      is_favorite: false,
    ...
    // デバッグ用ログ
    if (index < 3) {
      console.log(`[${index}] ${horse.name} - Base Horse:`, {
        id: baseHorse.id,
        name: baseHorse.name,
        sold_price: baseHorse.sold_price,
        is_unsold: baseHorse.is_unsold,
        weight: baseHorse.weight
      });
    }
    ...
    // 表示用の価格を取得する関数
    const getDisplayPrice = (price: any, isUnsold: boolean): string => {
      if (isUnsold) return '主取り';
      if (price === null || price === undefined || price === '') return '価格不明';
      
      // 数値に変換
      const numPrice = toNumeric(price);
      if (numPrice <= 0) return '主取り';
    ...
    // 賞金をフォーマットする関数
    const formatPrizeFromYen = (amount: number): string => {
      if (!amount || isNaN(amount)) return '賞金なし';
      if (amount >= 100000000) {
    ...
    // 更新された馬データを返す
    const resultHorse: AnalysisHorse = {
      ...baseHorse,
      roi: calculatedRoiValue,
      price_per_kg: pricePerKgValue,
      display_price: getDisplayPrice(soldPrice, isHorseUnsold),
      // 馬体重を数値のみで保持（表示時にkgを付与）
      weight: effectiveWeight || 0,  // 数値のみを保持
      display_weight: effectiveWeight !== null && effectiveWeight > 0 ? `${effectiveWeight}kg` : '-',
      ...
      primary_image: imageUrlNormalized,
      seller: effectiveAuction.seller || '',
      auction_date: effectiveAuction.auction_date || '',
      comment: effectiveAuction.comment || '',
    ...
    // デバッグ用ログ
    if (index < 3) {
      console.log(`[${index}] ${horse.name} - Result Horse:`, {
        display_price: resultHorse.display_price,
        display_weight: resultHorse.display_weight,
        display_prize: resultHorse.display_prize,
        display_roi: resultHorse.display_roi,
        is_unsold: resultHorse.is_unsold
      });
    }
    ...
    // 結果をフィルタリングして返す
    const filteredHorsesResult = result.filter((horse): horse is AnalysisHorse => {
      ...
      const isValid = 
        horse.id && 
        horse.name && 
        horse.effectiveAuction && 
        horse.effectiveAuction.id;
      
      if (!isValid) {
        console.warn('Invalid horse data filtered out:', horse);
        return false;
      }
    ...
    // 変換に失敗した馬がいる場合、最初の数件をログに出力
    if (filteredHorsesResult.length < horsesArray.length) {
      const failedCount = horsesArray.length - filteredHorsesResult.length;
      console.warn(`Failed to transform ${failedCount} horses. First few failures:`);
      
      // 失敗した馬の最初の3件をログに出力
      let count = 0;
      for (let i = 0; i < horsesArray.length && count < 3; i++) {
        const horse = horsesArray[i];
        const found = filteredHorsesResult.some(h => 
          (h.id && h.id === String(horse.id)) || 
          (h.name && h.name === horse.name)
        );
        
        if (!found) {
          console.warn(`Failed horse [${i}]:`, {
            id: horse.id,
            name: horse.name,
            reason: 'Invalid or missing required fields'
          });
          count++;
        }
      }
    ...
    // 変換に成功した馬が1件もない場合、デバッグ用のデータを返す
    if (filteredHorsesResult.length === 0 && horsesArray.length > 0) {
      console.error('No horses were transformed successfully. Creating debug horse data...');
      
      // 最初の馬のデータを使用してデバッグ用のデータを作成
      const firstHorse = horsesArray[0];
      const debugHorse: AnalysisHorse = {
        id: String(firstHorse.id || 'debug-1'),
        name: firstHorse.name || 'デバッグ用の馬',
        age: typeof firstHorse.age === 'number' ? firstHorse.age : (parseInt(firstHorse.age, 10) || 3),
        sex: firstHorse.sex || '牡',
        sire: firstHorse.sire || '不明',
        dam: firstHorse.dam || '不明',
        damsire: firstHorse.damsire || '不明',
        image_url: firstHorse.image_url || '',
        jbis_url: firstHorse.jbis_url || '',
        auction_url: firstHorse.auction_url || '',
        disease_tags: Array.isArray(firstHorse.disease_tags) ? firstHorse.disease_tags : [],
        created_at: firstHorse.created_at || new Date().toISOString(),
        updated_at: firstHorse.updated_at || new Date().toISOString(),
        auction_date: firstHorse.auction_date || new Date().toISOString().split('T')[0],
        sold_price: 1000,
        seller: firstHorse.seller || '不明',
        weight: firstHorse.weight || 450,
        total_prize_start: 0,
        total_prize_latest: 1000000,
        unsold: false,
        unsold_count: 0,
        comment: 'デバッグ用のデータです',
        roi: 0,
        price_per_kg: 0,
        display_price: '¥1,000',
        display_weight: '450kg',
        display_prize: '¥100万',
        display_roi: '0.0%',
        sort_price: 1000,
        sort_prize: 1000000,
        sort_roi: 0,
        primary_image: firstHorse.image_url || '',
        detail_url: firstHorse.detail_url || firstHorse.auction_url || `#/horse/${firstHorse.id || 'debug-1'}`,
        auction_history: [],
        is_auction: false,
        is_new: false,
        is_updated: false,
        ...
        error_message: '',
        effectiveAuction: {
          id: `debug-auction-${firstHorse.id || '1'}`,
          horse_id: String(firstHorse.id || 'debug-1'),
          auction_date: firstHorse.auction_date || new Date().toISOString().split('T')[0],
          sold_price: 1000,
          total_prize_start: 0,
          total_prize_latest: 1000000,
      ...
      console.log('Debug horse data created:', debugHorse);
      return [debugHorse];
    }
    ...
    // エラーが発生した場合でも、空の配列ではなくデバッグ用のデータを返す
...
const AnalysisContent = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch('/static/data/horses.json');
      if (!response.ok) {
        throw new Error('データの読み込みに失敗しました');
      }
      
      const horsesArray = await response.json();
      
      console.log('Raw horses data:', horsesArray);
      
      // データを変換（horsesプロパティに配列を設定）
      const transformedHorses = transformHorseData({ horses: horsesArray });
  ...
  if (showType === 'sold') {
    ...
    const avgPrice = data.metadata.average_price;
