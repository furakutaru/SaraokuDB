...
class ScraperConfig:
    ...
    # PC向けUser-Agentのリスト（モバイル/タブレットは除外）
    PC_USER_AGENTS = [
        # Windows + Chrome
    ...
    def get_random_user_agent(self) -> str:
        """ランダムなPC向けUser-Agentを取得"""
        return random.choice(self.PC_USER_AGENTS)
        
    def get_next_user_agent(self) -> str:
        """次のPC向けUser-Agentをローテーションして取得"""
...
{{ ... }}
            ...
            # リクエストを実行 (timeoutは既にkwargsに含まれている可能性があるので削除)
            kwargs.pop('timeout', None)  # 重複を避けるために一度削除
            response = original_request(method, url, **kwargs)
            
            # モバイルサイトにリダイレクトされていないかチェック
            if 'location' in response.headers and 'm.' in response.headers['location']:
                self.logger.warning("モバイルサイトへのリダイレクトを検出しました。リダイレクト先: %s", response.headers['location'])
                # リダイレクトを無効化して再試行
                kwargs['allow_redirects'] = False
                response = original_request(method, url, timeout=timeout, **kwargs)
