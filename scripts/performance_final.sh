...
RESULTS_FILE="performance_results_fixed_$(date +%Y%m%d_%H%M%S).csv"

echo "=== SaraokuDB API パフォーマンス測定（修正版） ==="
echo "測定開始時刻: $(date)"
...
# 正しいURL（末尾にスラッシュ）
API_URL="$API_BASE_URL/api/horses/"
...
for i in {1..3}; do
    ...
    response=$(curl -s -w "%{http_code}" -o /dev/null "$API_URL?limit=24&skip=0&sort=price_desc")
...
for sort in "${sort_types[@]}"; do
    ...
    for i in {1..2}; do
        start_time=$(get_timestamp_ms)
        response=$(curl -s -w "%{http_code}" -o /dev/null "$API_URL?limit=24&skip=0&sort=$sort")
        end_time=$(get_timestamp_ms)
        response_time=$((end_time - start_time))
...
for page in {0..2}; do
    ...
    for i in {1..2}; do
        start_time=$(get_timestamp_ms)
        response=$(curl -s -w "%{http_code}" -o /dev/null "$API_URL?limit=24&skip=$skip&sort=price_desc")
        end_time=$(get_timestamp_ms)
        response_time=$((end_time - start_time))
